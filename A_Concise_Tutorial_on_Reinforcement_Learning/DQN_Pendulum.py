...
next_model.load_state_dict(model.state_dict())  # 初始化权重相同
...
def compute_target_q(next_model, rewards, next_states, dones, gamma=0.99):
    """计算目标 Q 值: r + γ * max_a' Q'(s',a')"""
    with torch.no_grad():
        max_next_q = next_model(next_states).max(1)[0].unsqueeze(1)
...
def train():
    ...
    for episode in range(200):
        ...
        for step in range(200):
            ...
            target_q = compute_target_q(next_model, rewards, next_states, dones, gamma)
    ...
    # 保存模型
    # torch.save(model.state_dict(), 'dqn_pendulum.pth')
    print("训练完成，模型未保存。")
